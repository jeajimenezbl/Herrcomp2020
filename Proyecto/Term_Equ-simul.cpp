...
const double Lx=60, Ly=60; //dimensiones de caja
const int Nx=5, Ny=5, N=Nx*Ny; //número de moleculas en x y y.
...
int main(void){
  ...
  for(ix=0;ix<Nx;ix++)
    for(iy=0;iy<Ny;iy++){
      ...
      //-----------------------(   x0,   y0,          Vx0,          Vy0, m0,R0)
      Molecula[Nx*iy+ix].Inicie((ix+1)*dx,(iy+1)*dy,V0*cos(Theta),V0*sin(Theta), m0,R0);//OJO
    }
